...
class Property extends CI_Controller
{
	...
	{
		$data = array(
			'title'       => 'Pilih Jenis Property',
			'content'     => 'myaccount/property/create_property'
		);
		$this->load->view('myaccount/layout/wrapp', $data, FALSE);
	}
	// Create Iklan Property Rumah
			...
			if ($this->form_validation->run()) {
				...
				if (!$this->upload->do_upload('property_image')) {
					...
					$data = [
						...
						'province'      => $province,
						'content'       => 'myaccount/property/create_rumah'
					];
					$this->load->view('myaccount/layout/wrapp', $data, FALSE);
				} else {
	...
	public function tanah()
	{
		...
		if ($user->post_count == 0) {
			...
			$data = [
				...
				'keywords'      => 'keywords',
				'province'      => $province,
				'content'       => 'myaccount/property/create_tanah'
			];
			$this->load->view('myaccount/layout/wrapp', $data, FALSE);
		}
	}
		...
		if ($user->post_count == 0) {
			...
			if ($this->form_validation->run()) {
				...
				if (!$this->upload->do_upload('property_image')) {
					...
					$data = [
						'title'        => 'Buat Iklan Ruko',
						'error_upload' => $this->upload->display_errors(),
						'province'      => $province,
						'content'       => 'myaccount/property/create_ruko'
					];
					$this->load->view('myaccount/layout/wrapp', $data, FALSE);
				} else {
		...
		if ($user->post_count == 0) {
			...
			if ($this->form_validation->run()) {
				...
				if (!$this->upload->do_upload('property_image')) {
					...
					$data = [
						'title'        => 'Buat Iklan Apartemen',
						'error_upload' => $this->upload->display_errors(),
						'province'      => $province,
						'content'       => 'myaccount/property/create_apartemen'
					];
					$this->load->view('myaccount/layout/wrapp', $data, FALSE);
				} else {
					...
					$data = [
						...
						'property_bed'              => $this->input->post('property_bed'),
						'property_bath'             => $this->input->post('property_bath'),
						'property_facility'       => $this->input->post('property_facility'),
						'property_price'            => $price_final,
						'property_negotiable'       => $this->input->post('property_negotiable'),
						'property_desc'             => $this->input->post('property_desc'),
						'property_image'            => $upload_data['uploads']['file_name'],
	...
	public function update_rumah($id)
	{
		...
		$image	  = $this->images_model->image_detail_property($id);
				...
				if ($valid->run()) {
					...
					if (!empty($_FILES['property_image']['name'])) {
						...
						if (!$this->upload->do_upload('property_image')) {
							...
							//End Validasi
							$data = [
								'title'        		=> 'Edit Property Rumah',
								'property'       	=> $property,
								'province'				=> $province,
								'image'				=> $image,
								'error_upload' 		=> $this->upload->display_errors(),
								'content'          => 'myaccount/property/update_rumah'
							];
							$this->load->view('myaccount/layout/wrapp', $data, FALSE);
					...
					if (!empty($_FILES['property_image']['name'])) {
						...
						if (!$this->upload->do_upload('property_image')) {
							...
							$data = [
								...
								'image'				=> $image,
								'error_upload' 		=> $this->upload->display_errors(),
								'content'          => 'myaccount/property/update_tanah'
							];
							$this->load->view('myaccount/layout/wrapp', $data, FALSE);
	...
	// Update Ruko
	public function update_ruko($id)
	{
		$user_id = $this->session->userdata('id');
		$province = $this->province_model->get_province();
		$property = $this->property_model->property_detail($id);
				...
				if ($valid->run()) {
					...
					if (!empty($_FILES['property_image']['name'])) {
						...
						if (!$this->upload->do_upload('property_image')) {
							...
							//End Validasi
							$data = [
								'title'        		=> 'Edit Property Ruko',
								'property'       	=> $property,
								'province'				=> $province,
								'image'				=> $image,
								'error_upload' 		=> $this->upload->display_errors(),
								'content'          => 'myaccount/property/update_ruko'
							];
							$this->load->view('myaccount/layout/wrapp', $data, FALSE);
	...
	public function update_apartemen($id)
	{
		$user_id = $this->session->userdata('id');
		$province = $this->province_model->get_province();
		$property = $this->property_model->property_detail($id);
		...
		if ($property->type_id == 4) {
			...
			if ($property->user_id == $user_id) {
				...
				if ($valid->run()) {
					...
					if (!empty($_FILES['property_image']['name'])) {
						...
						if (!$this->upload->do_upload('property_image')) {
							...
							//End Validasi
							$data = [
								'title'        		=> 'Edit Property Apartemen',
								'property'       	=> $property,
								'province'				=> $province,
								'image'				=> $image,
								'error_upload' 		=> $this->upload->display_errors(),
								'content'          => 'myaccount/property/update_apartemen'
							];
							$this->load->view('myaccount/layout/wrapp', $data, FALSE);
				...
				//End Masuk Database
				$data = [
					'title'        			=> 'Update Property Apartemen',
					'property'       		=> 	$property,
					'image'				=> $image,
					'province'					=> $province,
					'content'          	=> 'myaccount/property/update_apartemen'
	...
	public function delete_image($id)
	{
		...
		$image	  = $this->images_model->image_detail($id);
		$user_id = $this->session->userdata('id');
		...
		if ($image->user_id == $user_id) {
			...
			if ($image->images != "") {
				unlink('./assets/img/property/' . $image->images);
				// unlink('./assets/img/artikel/thumbs/' . $berita->berita_gambar);
			}
			//End Hapus Gambar
			$data = ['id'   => $image->id, 'user_id' => $user_id];
			$this->images_model->delete($data);
			$this->session->set_flashdata('message', '<div class="alert alert-success">Gambar Telah di hapus</div>');
